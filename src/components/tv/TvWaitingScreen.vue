...
<template>
  <div class="flex flex-col items-center justify-center h-full gap-8">
    <!-- Screen name pill -->
    <div v-if="screenName"
      class="absolute top-[1.5vw] left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-sm rounded-full px-[1.5vw] py-[0.5vw]">
      <span class="text-white/70 text-tv-label font-condensed uppercase tracking-wider">{{ screenName }}</span>
    </div>
    ...
    <div class="text-center">
      <p class="text-tv-heading font-black font-condensed text-white">
        {{ timeString }}
      </p>
      <p class="text-tv-label text-white/60 font-condensed uppercase tracking-wider mt-2">
        {{ dateString }}
      </p>
    </div>
    ...
    <div class="h-16 flex items-center justify-center">
      <Transition name="tv-phrase" mode="out-in">
        <p v-if="showPhrase" :key="currentPhraseIndex"
          class="text-tv-body text-gymOrange font-black font-condensed uppercase tracking-wider">
